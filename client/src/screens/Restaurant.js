...
const ProfileElem = ({ profilePicture, rating }) => {
    ...
    return (
        <View style={styles.hasEatenElementWrapper}>
            <Image style={styles.hasEatenPicture} source={{ uri: profilePicture }} />
            {rating && (
    ...
    useEffect(() => {
        const fetchFriendsRatings = async () => {
            try {
                ...
                const { data, rating } = await ApiCaller('/ratingOfAResto', token, reqBody);
            ...
            <View style={styles.mainContainer}>
                <ScrollView style={styles.outerWrapper} bounces={true}>
                    <View style={styles.backdropWrapper}>
                        <Image
                            source={{
                                uri: restoData?.Restaurant
                                    ? restoData.Restaurant.photo
                                    : restoData.photo,
                            }}
                    ...
                    >
                        <Text style={{ ...styles.contentTitle, ...styles.white }}>
                            {restoData?.Restaurant ? restoData.Restaurant.title : restoData.title}
                        </Text>
                        <Text
                            style={{
                        ...
                        <View style={styles.hasEatenContainer}>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    <ProfileElem profilePicture={item.User.profilePicture} />
                                )}
                                keyExtractor={(item) => item.id}
                                horizontal={true}
                                style={styles.hasEatenElements}
